...
### ⚠ POST-HOC AMENDMENT — 2026-08-13, Phase 0 protocol v2

**Everything in this block was decided AFTER seeing episode 1 fire the kill
rule.** It is not pre-registered in the sense the rest of this document is, and
any reader should discount it accordingly. It is recorded here, separated and
labelled, rather than folded silently into §5 — the whole point of §12 is that
a reader can see which decisions preceded the data and which followed it.

The superseded result stands published at `RESULTS-phase0.md` and is not
revised in light of anything found under the amended protocol.

**Trigger.** Phase 0 v1 returned `T_null = 0.995` against a `0.0275` threshold.
The finding was verified as genuine (a pure permutation of identical sentences
flips P(correct) from 0.990 to 0.005), but v1 could not distinguish a general
property of ablation-based attribution from a single degenerate episode, and
its `T_null` statistic did not mean what it claimed.

**A1. Multi-episode noise floor.** N1/N2/REF now run over **12 independent
seeded episodes** rather than one. Statistics are pooled across episodes before
the kill rule is applied, and the per-episode distribution is reported so
heterogeneity stays visible. *Rationale:* §4a of the results document — one
episode cannot separate "the method is fragile" from "this instance is
bistable," and those imply opposite next steps.

**A2. `T_null` is pooled so the percentile is a percentile.** With 6 orders,
one episode yields 15 order-pair TVs plus 1 paraphrase TV; nearest-rank p95
over 16 values is the maximum, making the rule operationally "does any pair
exceed." Pooling 12 episodes gives 192 values, where p95 is rank 183 — an
actual percentile. **The 95th-percentile statistic and the `0.5 x T_ablate`
threshold are unchanged**; only the sample it is computed over changes.
*Rationale:* §4b. This is deliberately the minimum edit that repairs the
statistic — loosening the threshold after a failed gate would be exactly the
manoeuvre this document exists to prevent.

**A3. Every action must be covered by at least one source.** The v1 environment
sampled each scout's patrol independently, so under `--overlap low` some routes
were covered by no scout at all. `east` was unmentioned by every report, giving
the listener a coherent "unruled-out, therefore clear" reading and creating the
bistability that drove the flips. Coverage is now generated by a cyclic
construction that guarantees the union of patrols is the full action set, at
both overlap levels. *Rationale:* §3. This contamination was flagged in advance
of the v1 run, which makes the fix a deferred correction rather than a
discovery — but it is still being applied after seeing it bite, and is labelled
accordingly.

**A4. Samples per arm reduced from 200 to 50.** v1 measured near-zero
within-order sampling variance: every arm returned P(correct) at 0.005, 0.875,
0.97, 0.99, or 1.000, never near 0.5. Binomial variance at those extremes is
small, so n=50 gives tighter intervals in practice than n=200 did at the
p=0.5 worst case the original figure was sized for. *Rationale:* empirical, and
it is what makes 12 episodes affordable (~6,600 calls, ~$12, versus ~26,400
calls and ~$48 at n=200). **This is a post-hoc reduction in statistical power
and is the amendment a skeptical reader should scrutinise hardest.** Arm sizes
and short arms are reported on every run so the realised power is auditable.

**A5. Bistability is reported as a first-class statistic.** Each episode
reports its maximum order-pair TV, and the run reports the fraction of episodes
exceeding 0.5. *Rationale:* the v1 pathology was not "orders drift a bit" but
"one order inverts the answer." A median-based summary would have hidden it
entirely; the aggregate must surface it by construction.

**A6. `T_ablate == 0` is relabelled INDETERMINATE rather than KILL.** The
literal rule returns KILL when ablation moves nothing, because `T_null >= 0`
holds trivially. But "ablating a whole source changed nothing, so there is no
signal to compare the noise floor against" is a different finding from "noise
exceeds signal", and reporting the first as the second would misstate why a run
stopped. *This is not a loosening:* INDETERMINATE halts exactly as KILL does,
and the set of outcomes permitting Phase 1 to begin is unchanged — only
`PROCEED` does that, and its condition is untouched.

**What is NOT amended:** the kill rule's form and threshold (§5.1), the value
function (§3.2), the operator semantics (§4.4), the gate criteria (§8), and
every claim in §2.3 about what the instrument cannot see. If the amended
protocol fires the kill rule again, the project stops.
